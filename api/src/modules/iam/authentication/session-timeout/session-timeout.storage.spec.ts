...
describe('SessionTimeoutStorage', () => {
  ...
  describe('validateSession', () => {
    ...
    it('should throw SessionExpiredError when idle timeout exceeded', async () => {
      ...
      const idleExceeded =
        now - (mockTimeoutConfig.idleTimeoutSeconds + 1) * 1000;
      mockRedisClient.hgetall.mockResolvedValue({
        createdAt: String(now - 60000),
  ...
  describe('getSessionCreatedAt', () => {
    it('should return createdAt timestamp', async () => {
      ...
      const result = await service.getSessionCreatedAt(userId, refreshTokenId);
  ...
  describe('deleteAllSessions', () => {
    ...
    it('should not throw on Redis error', async () => {
      ...
      await expect(service.deleteAllSessions(userId)).resolves.not.toThrow();
    });
  });
});
