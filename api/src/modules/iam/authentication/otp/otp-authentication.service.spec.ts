...
describe('OtpAuthenticationService', () => {
  ...
  beforeEach(async () => {
    ...
    mockEncryptionService = {
      encrypt: jest.fn((val) => Promise.resolve(`encrypted:${val}`)),
      decrypt: jest.fn((val) => Promise.resolve(val.replace('encrypted:', ''))),
  ...
  describe('verifyAndEnableTfa', () => {
    ...
    it('should enable TFA when code is valid', async () => {
      mockRedisService.get.mockResolvedValue(
        await mockEncryptionService.encrypt(pendingData),
      );
