...
export class AuthenticationService {
  ...
  async signIn(signInDto: SignInDto, context: AuditContext = {}) {
    const email = signInDto.email.toLowerCase().trim();

    // Prevents timing attacks and reduces load during brute force attempts
    ...
    if (!user.password) {
      await this.passwordLockoutStorage.recordFailure(email);
      await this.logLoginFailed(user.id, 'No password set', context, email);
      throw new UnauthorizedException('Invalid credentials');
    }
    ...
    if (!isEqual) {
      await this.recordLoginFailure(user.id, email, context);
      throw new UnauthorizedException('Invalid credentials');
    }
    ...
    if (user.isTfaEnabled) {
      ...
      if (signInDto.recoveryCode) {
        ...
        await this.recordLoginSuccess(user.id, email, context);
      ...
      if (!result.valid) {
        await this.logLoginFailed(user.id, 'Invalid OTP code', context, email);
  ...
  private async recordLoginFailure(
    ...
    await this.logLoginFailed(userId, 'Invalid password', context, email);
  ...
  private async recordLoginSuccess(
    ...
    try {
      ...
      this.logger.error(
        'Failed to log LOGIN_SUCCESS audit event',
  ...
  private async logLoginFailed(
    ...
    try {
      ...
      this.logger.error(
        'Failed to log LOGIN_FAILED audit event',
        auditError instanceof Error ? auditError.stack : String(auditError),
      );
    }
  }
  ...
  async changePassword(
    ...
    try {
      ...
      this.logger.error(
        'Failed to log PASSWORD_CHANGED audit event',
        auditError instanceof Error ? auditError.stack : String(auditError),
      );
    }
  }
  ...
  async signOut(
    ...
    try {
      ...
      this.logger.error(
        'Failed to log LOGOUT audit event',
        auditError instanceof Error ? auditError.stack : String(auditError),
      );
    }
  }
  ...
  async refreshTokens(
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          'Failed to log TOKEN_REFRESH audit event',
          auditError instanceof Error ? auditError.stack : String(auditError),
        );
      }
