...
export class KeyRotationService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const result = this.checkKeyRotation();
    this.logHealthStatus(result);
  }

  checkKeyRotation(): KeyRotationCheckResult {
  ...
  async validateKeyMetadata(keyPath: string): Promise<{
    ...
    try {
      ...
      if (!metadata.algorithm) {
        ...
        issues.push(`Unexpected algorithm: ${String(metadata.algorithm)}`);
      }
  ...
  generateRotationReport(): {
    ...
    const checkResult = this.checkKeyRotation();
