...
export class AuditService {
  ...
  async log(
    ...
    auditEntry.entityName = dto.entityName ?? null;
    ...
    auditEntry.signature = this.integrityService
      ? await this.integrityService.generateSignature(auditEntry)
      : '';
  ...
  async findAll(
    ...
    if (options.actorEmail) {
      qb.andWhere('actor.email ILIKE :actorEmail', {
  ...
  async validateIntegrity(options?: {
    ...
    if (this.integrityService?.isConfigured()) {
      ...
      const entries = await qb.getMany();
      signatureResult = await this.integrityService.verifyBatch(entries);
    }
