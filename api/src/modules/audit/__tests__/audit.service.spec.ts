...
const createMockIntegrityService = () => ({
  generateSignature: jest.fn().mockResolvedValue('mocksignature123'),
  verifySignature: jest.fn().mockResolvedValue({ valid: true, entryId: '1' }),
  verifyBatch: jest
    .fn()
    .mockResolvedValue({ valid: true, entriesChecked: 0, invalidEntries: [] }),
...
describe('AuditService', () => {
  ...
  describe('validateIntegrity', () => {
    it('should return combined hash chain and signature validation results', async () => {
      ...
      mockIntegrityService.verifyBatch.mockResolvedValue({
        valid: true,
        entriesChecked: 1,
        invalidEntries: [],
      });
