...
export class TemplateVersioningService {
  ...
  async publish(id: string, userId?: string): Promise<AssessmentTemplate> {
    ...
    try {
      await this.auditService.log(
        {
          eventType: AuditEventType.TEMPLATE_PUBLISHED,
          entityType: 'Template',
          entityId: id,
      ...
      this.logger.error(
        'Failed to log audit event for template publish',
