...
export class TemplatesService {
  ...
  async create(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      try {
        ...
        this.logger.error(
          'Failed to log audit event for template creation',
  ...
  async update(
    ...
    if (dto.description !== undefined) {
      ...
      newValues.description = template.description;
    }
    if (dto.effectiveFrom !== undefined) {
      oldValues.effectiveFrom = template.effectiveFrom;
      template.effectiveFrom = dto.effectiveFrom
        ? new Date(dto.effectiveFrom)
        : null;
      newValues.effectiveFrom = template.effectiveFrom;
    }
    if (dto.effectiveTo !== undefined) {
      oldValues.effectiveTo = template.effectiveTo;
      template.effectiveTo = dto.effectiveTo ? new Date(dto.effectiveTo) : null;
      newValues.effectiveTo = template.effectiveTo;
    }
    ...
    try {
      ...
      this.logger.error(
        'Failed to log audit event for template update',
        auditError instanceof Error ? auditError.stack : String(auditError),
      );
    }
      ...
      await this.auditService.log(
        {
          ...
          oldValues: { name: templateName, version: templateVersion },
        },
        { actorId: userId },
      );
    } catch (auditError) {
      this.logger.error(
        'Failed to log audit event for template deletion',
        auditError instanceof Error ? auditError.stack : String(auditError),
      );
    }
  }
