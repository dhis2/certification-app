...
import type { CriterionService } from '../services/criterion.service';
import type { TemplateVersioningService } from '../services/template-versioning.service';
...
describe('TemplatesController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new TemplatesController(
      ...
      mockCriterionService as unknown as CriterionService,
  ...
  describe('publish', () => {
    it('should publish template', async () => {
      mockVersioningService.publish.mockResolvedValue({
        ...mockTemplate,
        isPublished: true,
      });

      const result = await controller.publish(mockTemplate.id, mockActiveUser);

      expect(result.isPublished).toBe(true);
      expect(mockVersioningService.publish).toHaveBeenCalledWith(
        mockTemplate.id,
        mockActiveUser.sub,
      );
    });
  ...
  describe('createNewVersion', () => {
    it('should create new version of template', async () => {
      ...
      mockVersioningService.createNewVersion.mockResolvedValue(newVersion);
  ...
  describe('addCriterion', () => {
    it('should add criterion to category', async () => {
      ...
      mockCriterionService.addCriterion.mockResolvedValue({
        ...mockCriterion,
        ...criterionDto,
      });
      ...
      expect(mockCriterionService.addCriterion).toHaveBeenCalledWith(
        mockCategory.id,
        criterionDto,
      );
    });
  ...
  describe('updateCriterion', () => {
    it('should update criterion', async () => {
      ...
      mockCriterionService.updateCriterion.mockResolvedValue({
        ...mockCriterion,
        ...updateDto,
      });
  ...
  describe('deleteCriterion', () => {
    it('should delete criterion', async () => {
      mockCriterionService.deleteCriterion.mockResolvedValue(undefined);

      await controller.deleteCriterion(mockCriterion.id);

      expect(mockCriterionService.deleteCriterion).toHaveBeenCalledWith(
        mockCriterion.id,
      );
    });
  });
  ...
  describe('delete', () => {
    it('should delete unpublished template', async () => {
      ...
      await controller.delete(mockTemplate.id, mockActiveUser);
