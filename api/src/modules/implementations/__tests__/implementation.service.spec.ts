...
describe('ImplementationsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should return paginated connection when no options provided', async () => {
      repository.createQueryBuilder.mockReturnValue({
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        orderBy: jest.fn().mockReturnThis(),
        take: jest.fn().mockReturnThis(),
        getManyAndCount: jest.fn().mockResolvedValue([mockImplementations, 2]),
      });
      ...
      expect(result.edges[0].node).toEqual(mockImplementation);
      expect(repository.createQueryBuilder).toHaveBeenCalledWith('impl');
    });
    ...
    it('should filter by isActive when option provided', async () => {
      repository.createQueryBuilder.mockReturnValue({
        ...
        orderBy: jest.fn().mockReturnThis(),
        take: jest.fn().mockReturnThis(),
        getManyAndCount: jest.fn().mockResolvedValue([[mockImplementation], 1]),
      });
      ...
      const qb = getQb();
      expect(qb.where).toHaveBeenCalledWith('impl.isActive = :isActive', {
        isActive: true,
      });
    });
    ...
    it('should return inactive implementations when isActive is false', async () => {
      ...
      repository.createQueryBuilder.mockReturnValue({
        ...
        getManyAndCount: jest
          .fn()
          .mockResolvedValue([[inactiveImplementation], 1]),
      });
    ...
    it('should return empty connection when no implementations found', async () => {
      repository.createQueryBuilder.mockReturnValue({
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        orderBy: jest.fn().mockReturnThis(),
    ...
    it('should use createQueryBuilder with impl alias', async () => {
      repository.createQueryBuilder.mockReturnValue({
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        orderBy: jest.fn().mockReturnThis(),
      ...
      expect(auditService.log).toHaveBeenCalledWith(
        {
          ...
          entityType: 'Implementation',
          entityId: mockImplementationId,
          entityName: updateDto.name,
          action: AuditAction.UPDATE,
          oldValues: { name: mockImplementation.name },
  ...
  describe('remove', () => {
    it('should soft delete implementation by setting isActive to false', async () => {
      ...
      await service.remove(mockImplementationId, mockUserId);
