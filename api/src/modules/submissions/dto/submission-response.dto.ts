...
export class SubmissionCriterionDto {
  @ApiProperty()
  id!: string;
  ...
  static fromEntity(criterion: Criterion): SubmissionCriterionDto {
    const dto = new SubmissionCriterionDto();
    dto.id = criterion.id;
...
export class SubmissionCategoryDto {
  @ApiProperty()
  id!: string;
  ...
  @ApiProperty({ type: [SubmissionCriterionDto] })
  criteria!: SubmissionCriterionDto[];

  static fromEntity(category: AssessmentCategory): SubmissionCategoryDto {
    const dto = new SubmissionCategoryDto();
    dto.id = category.id;
    ...
    dto.criteria = (category.criteria || [])
      ...
      .map((c) => SubmissionCriterionDto.fromEntity(c));
    return dto;
  }
}
...
export class SubmissionTemplateDto {
  ...
  @ApiProperty({ type: [SubmissionCategoryDto] })
  categories!: SubmissionCategoryDto[];

  static fromEntity(template: AssessmentTemplate): SubmissionTemplateDto {
    const dto = new SubmissionTemplateDto();
    dto.id = template.id;
    ...
    dto.categories = (template.categories || [])
      .sort((a, b) => a.sortOrder - b.sortOrder)
      .map((c) => SubmissionCategoryDto.fromEntity(c));
    return dto;
  }
}
...
export class SubmissionResponseDto {
  ...
  @ApiPropertyOptional({ type: SubmissionTemplateDto })
  template?: SubmissionTemplateDto;
  ...
  static fromEntity(
    ...
    if (withDetails && submission.template) {
      dto.template = SubmissionTemplateDto.fromEntity(submission.template);
    }
