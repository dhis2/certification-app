  ...
  return [
    {
      ...
      password: process.env.SEED_DEV_ADMIN_PASSWORD ?? 'DevAdmin#2024!pwd',
      roleName: 'admin',
    },
    {
      firstName: 'User',
      ...
      password: process.env.SEED_DEV_USER_PASSWORD ?? 'DevUser#2024!pwd',
      roleName: 'user',
    },
  ];
}
...
async function hashPassword(password: string): Promise<string> {
  ...
  return hash(password.normalize('NFC'), salt);
}
